...
class SGMBSCBertEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if hasattr(bert_outputs, "last_hidden_state"):
        ...
        weight_adj = self.attn(gcn_inputs, gcn_inputs, short_mask, aspect, src_mask)
        gcn_outputs = gcn_inputs
        layer_list = [gcn_inputs]

        for i in range(self.layers):
            gcn_outputs = gcn_outputs.unsqueeze(1).expand(
                batch, self.attention_heads, seq_len, self.attdim
            )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, short, aspect, mask=None):
        ...
        return attention(query, key, short, aspect, self.weight_m, self.bias_m, mask=mask, dropout=self.dropout)
